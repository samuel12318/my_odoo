...
class FinanceTracker(models.Model):
    ...
    _last_balance_change = {}  # Store last change time for each record
...
class FinanceTransaction(models.Model):
    ...
    def _check_amount(self):
        for record in self:
            if record.amount < 0:
                raise exceptions.ValidationError("Amount cannot")
...
class FinanceGoals(models.Model):
    ...
    def _compute_status(self):
        for record in self:
            if record.current_amount==record.target:
                record.status=True
    ...
    def run_compute_amount(self):
        ...
        for record in goals:
            if record.next_run_date == date.today(): 
                if record.account_id.balance >= record.transfer_in:
